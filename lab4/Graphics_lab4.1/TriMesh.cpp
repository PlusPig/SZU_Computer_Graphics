...
void TriMesh::generateDisk(int num_division, float radius)
{
	...
	vertex_colors.push_back(glm::vec3(1, 1, 1));	// 白色
	vertex_normals.push_back(glm::vec3(0, 0, 1));	// 只需要垂直Z轴方向
	vertex_textures.push_back(glm::vec2(0.5, 0.5)); // 纹理中心

	// 这个只需要计算上下表面
	// 这个是上表面，需要拆分成三角形然后映射纹理照片
	for (int i = 0; i < num_samples; i++)
	{
		float r_r_r = i * step;
		float x = radius * cos(r_r_r);
		...
		vertex_normals.push_back(normalize(glm::vec3(0, 0, 1))); // 因为是平面，所以法向量直接指向z轴正方向
		vertex_colors.push_back(normalize(glm::vec3(1, 1, 1)));

		// 纹理坐标映射到[0,1]区间
		vertex_textures.push_back(glm::vec2(0.5 + 0.5 * cos(r_r_r), 0.5 + 0.5 * sin(r_r_r)));
	}

	for (int i = 1; i <= num_samples; i++)
	{
		int next = (i % num_samples) + 1;
		faces.push_back(vec3i(0, i, next));
	}
...
void TriMesh::generateCone(int num_division, float radius, float height)
{
	...
	vertex_textures.push_back(glm::vec2(0.0, 0.0)); // 纹理中心
	...
	// ---- 锥顶 ----
	// ---- 锥顶 ----
	int apex_index = num_samples + 1;
	vertex_positions.push_back(glm::vec3(0, 0, height));
	vertex_colors.push_back(glm::vec3(1, 1, 1));
	vertex_normals.push_back(glm::vec3(0, 0, 1));	// 近似
	vertex_textures.push_back(glm::vec2(0.5, 1.0)); // 仅用于侧面
	...
	// 	int next = (i % num_samples) + 1;
	// 	faces.push_back(vec3i(0, i, next));
	// 	// texture_index随便填，底面不用贴图
	// 	texture_index.push_back(vec3i(0, i, next));
	// }
	...
	{
		int next = (i % num_samples) + 1;
		faces.push_back(vec3i(i, apex_index, next));
		// 侧面纹理坐标：u用角度映射，v用高度
		int tex_base = vertex_textures.size();
		float u0 = 1.0f * (i - 1) / num_samples;
		float u1 = 1.0f * (next - 1) / num_samples;
		// 添加纹理坐标（每个侧面三角形独立分配纹理坐标）
		vertex_textures.push_back(glm::vec2(u0, 0.0f));	  // 当前圆周点
		vertex_textures.push_back(glm::vec2(0.5f, 1.0f)); // 顶点
		vertex_textures.push_back(glm::vec2(u1, 0.0f));	  // 下一个圆周点
		texture_index.push_back(vec3i(tex_base, tex_base + 1, tex_base + 2));
	}
