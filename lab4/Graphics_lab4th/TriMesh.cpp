...
void TriMesh::readObj(const std::string &filename)
{
	...
	while (std::getline(fin, line))
	{
		...
		if (sin >> type)
		{
			...
			else if (type == "f")
			{
				// 这里假设每个面片都是三角形
				// 每2个数字之间空一个间隔符
				// 这里一行的每3个数字分别表示点坐标，texture坐标，法向量坐标
				// 实验指导里说了f有可能是3个点的，也可能是4个点的
				sin >> a0 >> slash >> b0 >> slash >> c0 >> a1 >> slash >> b1 >> slash >> c1 >> a2 >> slash >> b2 >> slash >> c2;
				// 注意obj文件的索引是从1开始的，所以这里要减1
				// 提醒得好
	...
	if (vertex_colors.empty() && !vertex_normals.empty())
	{
