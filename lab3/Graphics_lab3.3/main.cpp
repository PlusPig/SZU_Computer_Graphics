...
void init()
{
	...
	// 设置光源位置
	light->setTranslation(glm::vec3(2.0, 2.0, 3.0));
	light->setAmbient(glm::vec4(1.0, 1.0, 1.0, 1.0));  // 环境光
	light->setDiffuse(glm::vec4(1.0, 1.0, 1.0, 1.0));  // 漫反射
	light->setSpecular(glm::vec4(1.0, 1.0, 1.0, 1.0)); // 镜面反射

	// 设置物体的旋转位移
	mesh->setTranslation(glm::vec3(0.5, 0.5, 0.5));
	mesh->setRotation(glm::vec3(0, 0.0, 0.0));
	mesh->setScale(glm::vec3(1.0, 1.0, 1.0));

	// 设置材质
	mesh->setAmbient(glm::vec4(0.2, 0.2, 0.2, 1.0)); // 环境光，较低的值以保持陶瓷的柔和感
	mesh->setDiffuse(glm::vec4(0.8, 0.8, 0.8, 1.0)); // 漫反射，较高的值以模拟陶瓷的反射特性
...
void display()
{
	...
	// 将模型矩阵和阴影矩阵相乘得到最终的阴影投影矩阵
	modelMatrix = shadowMatrix * modelMatrix;
	glUniform1i(mesh_object.shadowLocation, 1); // 表示绘制阴影
	glUniformMatrix4fv(mesh_object.modelLocation, 1, GL_FALSE, &modelMatrix[0][0]);
	// 表示以三角形方式绘制
	glDrawArrays(GL_TRIANGLES, 0, mesh->getPoints().size());
}
...
void mainWindow_key_callback(GLFWwindow *window, int key, int scancode, int action, int mode)
{
	...
	else if (key == GLFW_KEY_MINUS && action == GLFW_PRESS && mode == 0x0000) // 初始化材质
	{
		mesh->setAmbient(glm::vec4(0.1, 0.1, 0.1, 1.0)); // 环境光，较低的值以保持陶瓷的柔和感
		mesh->setDiffuse(glm::vec4(0.8, 0.8, 0.8, 1.0)); // 漫反射，较高的值以模拟陶瓷的反射特性
		mesh->setSpecular(glm::vec4(0.2, 0.2, 0.2, 1.0));
		mesh->setShininess(16.0); // 高光系数，中等值以产生适中的高光效果
	}
	...
	{
		glm::vec4 specular = mesh->getSpecular();
		specular.x = std::max(specular.x - 0.1f, 0.0f);
		mesh->setSpecular(specular);
	}
	...
	{
		glm::vec4 specular = mesh->getSpecular();
		specular.y = std::max(specular.y - 0.1f, 0.0f);
		mesh->setSpecular(specular);
	}
...
int main(int argc, char **argv)
{
	...
	GLFWwindow *mainwindow = glfwCreateWindow(600, 600, "lab3_2023152019_朱政灏", NULL, NULL);
	if (mainwindow == NULL)
	{
