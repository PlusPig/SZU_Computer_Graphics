...
void bindLightAndMaterial(TriMesh *mesh, openGLObject &object, Light *light, Camera *camera)
{
	...
	// 设置光照控制变量（默认全部关闭）
	glUniform1i(object.ambientOpen, 0);
	glUniform1i(object.DiffuseOpen, 0);
	glUniform1i(object.SpecularOpen, 0);
}
...
void mainWindow_key_callback(GLFWwindow *window, int key, int scancode, int action, int mode)
{
	...
	else if (key == GLFW_KEY_0 && action == GLFW_PRESS && mode == 0x0000)
	{
		...
		shininess = std::min(shininess + 1.0f, 100.0f);
...
int main(int argc, char **argv)
{
	...
	GLFWwindow *mainwindow = glfwCreateWindow(600, 600, "2023152019_朱政灏_lab3.4", NULL, NULL);
	if (mainwindow == NULL)
	{
