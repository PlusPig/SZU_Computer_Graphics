 ...
 * - 3) 绘制了‘L’、‘J’、‘I’、‘Cube’、‘Z’、‘S’、‘T’形状的方块的所有旋转方向
 * - 4) 随机生成不同形状的方块
 * - 5) 修改updatatitle函数控制进入方格的方块才会进行渲染
 * - 6) 修改rotate函数在棋盘格范围内正确控制当前方块控制方块的移动和旋转
 * - 7) 随机生成方块并赋上不同的颜色
 * - 8) 方块的自动向下移动
 * - 9) 方块之间、方块与边界之间的碰撞检测
 * - 10) 棋盘格中每一行填充满之后自动消除
...
glm::vec2 allRotationsIshape[4][4] =
	{{glm::vec2(-1, 0), glm::vec2(0, 0), glm::vec2(1, 0), glm::vec2(2, 0)}, //   "I"
	 {glm::vec2(0, -1), glm::vec2(0, 0), glm::vec2(0, 1), glm::vec2(0, 2)},
	 {glm::vec2(-1, 0), glm::vec2(0, 0), glm::vec2(1, 0), glm::vec2(2, 0)},
	 {glm::vec2(0, -1), glm::vec2(0, 0), glm::vec2(0, 1), glm::vec2(0, 2)}};

glm::vec2 allRotationsCubeshape[4][4] =
	{{glm::vec2(0, 1), glm::vec2(0, 0), glm::vec2(1, 0), glm::vec2(1, 1)}, //   "Cube"
...
glm::vec2 allRotationsSshape[4][4] =
	{{glm::vec2(1, 0), glm::vec2(0, 0), glm::vec2(0, 1), glm::vec2(-1, 1)}, //   "S"
	 {glm::vec2(-1, -1), glm::vec2(-1, 0), glm::vec2(0, 0), glm::vec2(0, 1)},
	 {glm::vec2(1, 0), glm::vec2(0, 0), glm::vec2(0, 1), glm::vec2(-1, 1)},
	 {glm::vec2(-1, -1), glm::vec2(-1, 0), glm::vec2(0, 0), glm::vec2(0, 1)}};
...
glm::vec4 colors[5] = {orange, white, purple, green, red}; // 集合颜色
...
void updatetile()
{
	...
	for (int i = 0; i < 4; i++)
	{
		...
		if (x >= 0 && x < board_width && y >= 0 && y < board_height)
		{
			...
			glBufferSubData(GL_ARRAY_BUFFER, vboIndex * 6 * sizeof(glm::vec4), 6 * sizeof(glm::vec4), newpoints);
			vboIndex++;
		}
	}
	glBindVertexArray(0);
...
void newtile()
{
	...
	case 3: // 方块
		for (int i = 0; i < 4; i++)
			tile[i] = allRotationsCubeshape[0][i];
		break;
	case 4: // Z
		for (int i = 0; i < 4; i++)
	...
	colorIndex = rand() % 5; // 随机颜色
	for (int i = 0; i < 24; i++)
	{
		newcolours[i] = colors[colorIndex];
		// newcolours[i] = orange; // 目前都用橙色
	}
...
bool checkvalid(glm::vec2 cellpos)
{
	...
	{
		if (board[(int)cellpos.x][(int)cellpos.y] == false)
			return true;
		else
			return false;
	}
...
void rotate()
{
	...
	{
	case 0:
		shapeArray = allRotationsLshape[nextrotation];
		break;
	case 1:
		shapeArray = allRotationsJshape[nextrotation];
		break;
	case 2:
		shapeArray = allRotationsIshape[nextrotation];
		break;
	case 3:
		shapeArray = allRotationsCubeshape[nextrotation];
		break;
	case 4:
		shapeArray = allRotationsZshape[nextrotation];
		break;
	case 5:
		shapeArray = allRotationsSshape[nextrotation];
		break;
	case 6:
		shapeArray = allRotationsTshape[nextrotation];
		break;
	}
	// 检查当前旋转之后的位置的有效性
	bool valid = true;
	for (int i = 0; i < 4; i++)
	{
		glm::vec2 pos = shapeArray[i] + tilepos;
		if (!checkvalid(pos))
		{
			valid = false;
			break;
...
void checkfullrow(int row)
{
	...
	for (int y = row; y < board_height - 1; y++)
	{
		for (int x = 0; x < board_width; x++)
		{
			...
			for (int k = 0; k < 6; k++)
			{
				board_colours[6 * (board_width * y + x) + k] = board[x][y] ? board_colours[6 * (board_width * (y + 1) + x) + k] : black;
			}
			changecellcolour(glm::vec2(x, y), board[x][y] ? board_colours[6 * (board_width * (y + 1) + x)] : black);
		}
	}
	// 最顶上一行清空
	for (int x = 0; x < board_width; x++)
...
void settile()
{
	...
	for (int i = 0; i < 4; i++)
	{
		...
		board[x][y] = true;
		glm::vec4 currentcolor = colors[colorIndex];
		changecellcolour(glm::vec2(x, y), currentcolor);
	}
...
int main(int argc, char **argv)
{
	...
	// 创建窗口。
	GLFWwindow *window = glfwCreateWindow(500, 900, "midtermlab_2023152019_朱政灏", NULL, NULL);
	if (window == NULL)
	{
	...
	init();
	double lastDropTime = glfwGetTime(); // 初始化下落时间
	while (!glfwWindowShouldClose(window))
	{
		double currentTime = glfwGetTime();
		// 每隔1秒自动下落一次
		if (!gameover && currentTime - lastDropTime >= 0.5)
		{
			if (!movetile(glm::vec2(0, -1)))
			{
				settile();
