...
void display()
{
	...
	// @TODO: Task-2 左小臂
	mstack.push(modelMatrix); // 把左大臂push进去
	...
	// @TODO: Task-2 右大臂
	mstack.pop();				// 现在剩下的是躯干
	modelMatrix = mstack.pop(); // 恢复躯干矩阵
	mstack.push(modelMatrix);	// 重新把躯干放进去
	modelMatrix = glm::translate(modelMatrix, glm::vec3(0.5 * robot.TORSO_WIDTH + 0.5 * robot.UPPER_ARM_WIDTH, robot.TORSO_HEIGHT, 0.0));
	...
	// @TODO: Task-2 右小臂
	mstack.push(modelMatrix); // 把右大臂push进去
	modelMatrix = glm::translate(modelMatrix, glm::vec3(0.0, -robot.UPPER_ARM_HEIGHT, 0.0));
	...
	// @TODO: Task-2 左小腿
	mstack.push(modelMatrix); // 把左大腿push进去
	...
	// @TODO: Task-2 右小腿
	mstack.push(modelMatrix); // 把右大腿push进去
	modelMatrix = glm::translate(modelMatrix, glm::vec3(0.0, -robot.UPPER_LEG_HEIGHT, 0.0));
...
int main(int argc, char **argv)
{
	...
	// 配置窗口属性
	GLFWwindow *window = glfwCreateWindow(600, 600, "2023152019_朱政灏_附加实验二", NULL, NULL);
	if (window == NULL)
	{
